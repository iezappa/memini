...
Memini. The release procedure is in [`RELEASING.md`](RELEASING.md).
...
which deletes every record Memini keeps on the device. A lost keystore means
...
Debug builds, `flutter run`, and the CI jobs (tests, integration, web) are unaffected.
...
```text
~/.android-keystores/memini/upload-keystore.jks
```

Generate it once, ever:

```bash
mkdir -p ~/.android-keystores/memini
chmod 700 ~/.android-keystores ~/.android-keystores/memini
keytool -genkey -v -keystore ~/.android-keystores/memini/upload-keystore.jks \
        -storetype JKS -keyalg RSA -keysize 2048 -validity 10000 -alias upload
chmod 600 ~/.android-keystores/memini/upload-keystore.jks
```
...
1. In the password manager, one entry for Memini with the `.jks` as an
...
keyPassword=...
keyAlias=upload
storeFile=/home/<you>/.android-keystores/memini/upload-keystore.jks
```
...
```bash
keytool -list -v -keystore ~/.android-keystores/memini/upload-keystore.jks -alias upload
# "SHA256: AB:CD:..."
```
